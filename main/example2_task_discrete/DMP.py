...
from utils.inverse_dynamics_models import (
    get2DOF_M,
    get2DOF_C,
    get2DOF_IK,
    get2DOF_J,
...
t_update = 1.0 / fps * speed  # Time for update
t_save = 1.0 / save_ps * speed  # Time for saving the data
...
for i, t in enumerate(td):
    A_mat[:, i] = (
        np.squeeze(fd.calc_multiple_ith(t, np.arange(N, dtype=int)))
        / fd.calc_whole_at_t(t)
        * sd.calc(t)
    )
...
for i in range(2):
    ...
    if (gd[i] - y0d[i]) != 0:
        W_LLS[i, :] = 1.0 / ((gd[i] - y0d[i])) * W_LLS[i, :]
    else:
        W_LLS[i, :] = 0
...
t_arr = np.append(
    t_arr, T
)  # Technical detail: Append the final to include T on the array
...
input_arr_discrete = fd.calc_forcing_term(
    t_arr[:-1], weight, t0, np.diag(g - y0), trimmed=True
)
